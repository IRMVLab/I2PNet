...
class ProjectPointNet(nn.Module):
    def __init__(self, H, W, out_h, out_w, stride_H, stride_W, kernel_size, nsample, distance, in_channel, mlp,
        ...
        for out_channel in mlp:
            ...
            #print("kernel:", last_channel, out_channel)
            last_channel = out_channel

    def forward(self, xyz_proj_raw, xyz_proj, feature_proj, sample_idx=None,cfg=cfg_default, featmode = None, raw_feat_point=False):
        """
        Input:
        ...
        if featmode == "dist":
            new_points = new_points = torch.cat(
            [dist, grouped_points], -1)
        else:
    ...
    def forward_dist(self, xyz_proj_raw, xyz_proj, feature_proj, sample_idx=None,cfg=cfg_default, featmode = None, raw_feat_point=False):
        """
        Input:
            xyz_proj_raw: B,H,W,3
            xyz_proj: B,H,W,3
...
class ProjSetUpconvModule(nn.Module):
    def __init__(self, H, W, out_h, out_w, stride_H, stride_W, kernel_size, nsample, distance, in_channels, mlp,
        ...
        self.nsample = nsample
        self.last_channel = in_channels[-1]
        self.mlp = mlp
        self.mlp2 = mlp2
        self.H = H
        self.W = W
    ...
    def forward(self, xyz1_raw, xyz2_raw, xyz1, xyz2, idx_n2, feat1, feat2,cfg=cfg_default, featmode = None, raw_feat_point=False):
        '''
            Input:
        ...
        dist = torch.norm(xyz_diff, p=2, dim=3).unsqueeze(3)

        #    [grouped_xyz_norm, dist], -1)
        if featmode == "dist":
            upfeats = torch.cat([grouped_feat2, dist], dim=3)  # B,N,K,1+C
        else:
