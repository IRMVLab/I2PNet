...
class nuScenesLoader(data.Dataset):
    def __init__(self, params, use_raw = False):
        ...
        val_info_path = os.path.join('/dataset/nus_processed_CMRNet', f'val_dataset_map_demo.list')
        test_info_path = os.path.join('/dataset/nus_processed_CMRNet', f'val_dataset_map_demo.list')
        ...
        #  199 199  200 197 198 200
        # scene-0922 scene-0981
        self.length = 197#199
    ...
    def __getitem__(self, index):
        ...
        lc_path, K, velo2cam2, extra = self.dataset[index * self.skip + 199 + 199 + 200]
        self.index = index
        self.dt = 1
        ...
        #print(len(extra))
        poses = extra[4].numpy()
        if not os.path.exists(os.path.join('/dataset/nus_processed_CMRNet/sequences', extra[5], 'image')):
            os.mkdir(os.path.join('/dataset/nus_processed_CMRNet/sequences', extra[5], 'image'))
        ...
        (Image.open(os.path.join(self.root, cp))).save(os.path.join('/dataset/nus_processed_CMRNet/sequences', extra[5], 'image', f'{index:06d}.jpg'))
...
if __name__ == '__main__':
    ...
    for index in range(0, 199):
        lc_path, K, velo2cam2, extra = dataset[index]
        Pcs.append(velo2cam2)
    np.save('/dataset/nus_processed_CMRNet/sequences/scene-0922/Pc', Pcs)
