...
def tvector2mat(t):
    ...
    """
    assert t.shape == (3,), "Not a valid translation"
    mat = np.eye(4)
    mat[0, 3] = t[0]
    mat[1, 3] = t[1]
    mat[2, 3] = t[2]
...
def load_map(seq, root_path, start, end):
    map_path = os.path.join(root_path, 'kitti_processed_CMRNet_small',
...
def make_dataset(root_path, mode, h5=False, larger=False):
    ...
    for seq in seq_list:
        ...
        #with open(os.path.join(root_path, 'kitti_processed_CMRNet_small', 'poses', f'kitti-{seq:02d}.csv')) as f:
            for x in f:
                if x.startswith('timestamp'):
                    continue
                x = x.split(',')
                T = np.array([float(x[1]), float(x[2]), float(x[3])])
                R = np.array([float(x[7]), float(x[4]), float(x[5]), float(x[6])])
                R = quat2mat(R)
                T = tvector2mat(T)
                RT = T @ R
                poses.append(RT) 
        ...
        maps = load_map(seq, root_path, 0, scans)
        ...
        for i in range(0, scans):
            ...
            dataset.append(
                (pc_path, img_path1, calib_path, seq, (map_seq_idx, map_idx), np.array(poses[i], np.float32), i, times[i]))
...
class Kitti_Odometry_Dataset(Dataset):
    ...
    def update_pose(self, pred_decalib_extrinsic):
        #print(pred_decalib_extrinsic.shape)
        if self.index > 0:
            # self.Pv = pred_decalib_extrinsic[0] / self.dt
            self.Pv_gt = self.pose_gt @ np.linalg.inv(self.gt_prev) / self.dt
            if self.index ==1:
                #print('P01',self.Pv_gt)
                self.Pv = self.Pv_gt#np.eye(4)
            self.Pv =  (self.Pprev @ np.linalg.inv(self.Pc) @ np.linalg.inv(pred_decalib_extrinsic[0]) @ self.Pc @ self.Pprev_inv / self.dt) @ self.Pv
            #print('Pv', self.Pv)
            #self.Pv =  self.Pprev @ np.linalg.inv(self.Pc) @ pred_decalib_extrinsic[0] @ self.Pc @ self.Pprev_inv / self.dt
            #self.Pv = np.linalg.inv(self.Pc) @ pred_decalib_extrinsic[0] @ self.Pc
        self.Pprev_inv = np.linalg.inv(self.Pc) @ pred_decalib_extrinsic[0] @ self.Pc @ self.Pprev_inv
        self.Pprev = np.linalg.inv(self.Pprev_inv)
        self.gt_prev = self.pose_gt
        return np.array([self.Pprev])
    ...
    def __getitem__(self, index):
        ...
        pc_folder, img_folder, calib_path, seq, map_idx, poses, seq_i, timestamp = self.dataset[index]
        ...
        if index == 0:
            ...
            #Pr = np.eye(4)
            P01 = np.array([[ 0.9999945,  -0.00291988, -0.00153735,  0.7471781 ],
                    [ 0.00291562,  0.9999919,  -0.00276717,  0.04331756],
                    [ 0.00154542,  0.00276267,  0.999995,   -0.06615829],
                    [ 0. ,         0.  ,        0.  ,        1.        ]])
            Pr = Pc @ P01 @ np.linalg.inv(Pc)
            Pr_inv = np.linalg.inv(Pr)
            pose_map = np.linalg.inv(Pc) @ Pr_inv @ Pc @ np.linalg.inv(poses)
            self.Pprev_inv = pose_map
        elif index ==1 :
            pose_map = self.Pprev_inv
            Pr_inv = Pc @ pose_map @ poses @ np.linalg.inv(Pc)
            Pr = np.linalg.inv(Pr_inv)
            #Pr = pose_map @ np.linalg.inv(poses)
            #pose_map = self.gt_prev
            #pose_map = poses
            self.Pprev_inv = pose_map
        else:
            #pose_map = np.linalg.inv(self.dt * self.Pv_gt @ self.Pprev) 
            pose_map = np.linalg.inv(self.dt * self.Pv @ self.Pprev) 
            #pose_map = self.Pprev_inv
            Pr_inv = Pc @ pose_map @ poses @ np.linalg.inv(Pc)
            Pr = np.linalg.inv(Pr_inv)
            #Pr = pose_map @ np.linalg.inv(poses)
            #pose_map = self.Pprev
            #pose_map = self.dt * self.Pv_gt @ self.gt_prev
            #pose_map = poses
            self.Pprev_inv = pose_map

        if self.use_map:
            global_map = self.maps
            # generate the local map runtime
            # forward pose
            #pose_map = np.linalg.inv(pose_map)
            # inverse pose [R^T,-R^T@t]
            local_pc = np.asarray(global_map.points).T  # (3,N)
            #print('init map shape', local_pc.shape)
            local_pc = np.concatenate([local_pc, np.ones((1, local_pc.shape[1]))], axis=0)
            # voxelized = voxelized.to(args.device)
            local_intensity = np.asarray(global_map.colors)[:, 0:1].T  # (1,N)

            local_pc = (pose_map @ local_pc)[:3, :]

            #print('map shape', local_pc.shape)
            indexes = local_pc[1] > -10.
            indexes = indexes & (local_pc[1] < 10.)
            indexes = indexes & (local_pc[0] > -5.)
            indexes = indexes & (local_pc[0] < 15.)
