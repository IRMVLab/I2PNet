...
parser.add_argument('--modelcfg', default="config_proj",type=str)
...
class Trainer(object):
    def __init__(self):
        ...
        if CKPT is not None:
            ...
            shutil.copy(f"src/{MODELCFG}.py", "{0}/config.py".format(LOGDIR))
        else:
            shutil.copy("src/{0}.py".format(dataset_file), "{0}/dataset.py".format(LOGDIR))
            shutil.copy("src/{0}.py".format(NETWORK), "{0}/network.py".format(LOGDIR))
            shutil.copy(f"src/{MODELCFG}.py", "{0}/config.py".format(LOGDIR))
        ...
        dataset = traindataset(cfg.dataset_params)
        self.train_loader = DataLoader(dataset,
                                       ...
                                       drop_last=True)
        # validation data
        dataset_valid3 = traindataset(cfg.dataset_params_valid3)
    ...
    def validate(self, n_iter):
        ...
        evaluator = None
        if "kd" in DATASET or "nus" in DATASET or "realloc" in DATASET:
        ...
        with torch.no_grad():
            for count, data_valid in tqdm(enumerate(self.test_loader3), total=len(self.test_loader3)):
                ...
                out3, out4, pm3, pm4, sx, sq = self.model(rgb_img, lidar_img, lidar_img_raw,
                                                          H_initial, intrinsic, resize_img,
                                                          gt_project, calib, lidar_feats,cfg=modelcfg)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, EPOCHS):
            ...
            running_rre_loss = AverageMeter()
            running_rte_loss = AverageMeter()
            ...
            for i, data in enumerate(tqdm(self.train_loader, total=len(self.train_loader))):
                ...
                lidar_img_raw = data["raw_point_xyz"].to(self.device).float()
                gt_project = None
                ...
                out3, out4, pm3, pm4, sx, sq = self.model(
                    rgb_img, lidar_img, lidar_img_raw, H_initial, intrinsic, resize_img,
                    gt_project, calib, lidar_feats,cfg=modelcfg)
                ...
                rre_run, rte_run = cal_rete_once(out3, data)
                running_rre_loss.update(float(rre_run))
                running_rte_loss.update(float(rte_run))

                if running_loss.count == SAVE_RATE:
                    n_iter = n_iter + 1
                    self.f_write.write(
                        'Epoch: {:5d} | Batch: {:5d} | Loss: {:03f}| realLoss: {:03f} | dualLoss: {:03f} | sx: {:03f} |'
                        ' sq: {:03f} | p_loss: {:03f} | rre: {:02f}| rte: {:02f}| lr: {:05f}\n'
                            .format(self.epoch + 1, n_iter, running_loss.avg, running_real_loss.avg,
                                    ...
                                    running_rre_loss.avg, running_rte_loss.avg,
                    ...
                    running_rre_loss.reset()
                    running_rte_loss.reset()
