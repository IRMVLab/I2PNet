...
class Evaluator(object):
    def __init__(self):
        ...
        print("Load Map...")
        map_path = os.path.join("/dataset/waymo/val_proc/00", "map_0.1.pcd")
        dpath = os.path.join("/dataset/waymo/val_proc/00", "map_0.1.pcd")
    ...
    def decode_meta(self, meta_info, seq):
        ...
        imp = os.path.join("/dataset/waymo/val_proc/00/images",
                           "%06d" % int(seq) + ".png")
        # pcl = np.fromfile(pcp, dtype=np.float32).reshape(-1, 4)[:, :3]
        img = cv2.imread(imp)
        K = np.asarray([[2.05904714e+03, 0.00000000e+00, 9.35124808e+02],
                        [0.00000000e+00, 2.05904714e+03, 6.35052475e+02],
                        [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])
        Pc = np.asarray([[-1.7268e-03, -9.9988e-01, 1.5125e-02, -5.2593e-02],
                         [2.8620e-03, -1.5130e-02, -9.9988e-01, 2.1103e+00],
                         [9.9999e-01, -1.6833e-03, 2.8878e-03, -1.5501e+00],
    ...
    def vis(self):
        ...
        for vis_t in tqdm(range(self.num)):
            ...
            if video_project is None:
                video_project = cv2.VideoWriter(f"waymo_project.mp4",
            ...
            indexes = local_map[:, 0] > -25.
            indexes = indexes & (local_map[:, 0] < 25.)
            indexes = indexes & (local_map[:, 1] > -10.)
            indexes = indexes & (local_map[:, 1] < 100.)
