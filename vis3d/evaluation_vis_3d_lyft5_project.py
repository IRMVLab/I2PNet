...
class Evaluator(object):
    def __init__(self):
        ...
        print("Load Map...")
        map_path = os.path.join("/dataset/lyft5/sequences/host-a004-lidar0-1233944976297786786-1233945001198600096/map",
                                'map-host-a004-lidar0-1233944976297786786-1233945001198600096_0.1.pcd')
        dpath = os.path.join("/dataset/lyft5/sequences/host-a004-lidar0-1233944976297786786-1233945001198600096/map",
                             'map-host-a004-lidar0-1233944976297786786-1233945001198600096_0.1.pcd')
        ...
        self.Pcs = np.load("/dataset/lyft5/sequences/host-a004-lidar0-"
                           "1233944976297786786-1233945001198600096/Pc.npy",allow_pickle=True)
    ...
    def decode_meta(self, meta_info, seq):
        ...
        imp = os.path.join("/dataset/lyft5/sequences/host-a004-lidar0-"
                           "1233944976297786786-1233945001198600096/image",
                           "%06d" % int(seq) + ".jpg")
        # pcl = np.fromfile(pcp, dtype=np.float32).reshape(-1, 4)[:, :3]
        img = cv2.imread(imp)
        K = np.asarray([[881.43876801, 0, 624.59698979],
                        [0.0, 881.43876801, 526.081830325],
                        [0.0, 0.0, 1.0]])
        Pc = self.Pcs[int(seq)].numpy()
    ...
    def vis(self):
        ...
        for vis_t in tqdm(range(self.num)):
            ...
            if video_project is None:
                video_project = cv2.VideoWriter(f"lyft5_project.mp4",
            ...
            local_map = self.transform(pcl, gt_extrinsic, inv=True)
            ...
            ppcl, pcolor = self.get_color_pc(self.transform(pcl, pred_extrinsic, inv=True),
                                             Pc, img, intrinsic)
