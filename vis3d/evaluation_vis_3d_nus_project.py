...
class Evaluator(object):
    def __init__(self):
        ...
        print("Load Map...")
        map_path = os.path.join("/dataset/nus_processed_CMRNet/sequences/scene-0922/map",
                                "map-scene-0922_0.1.pcd")
        dpath = os.path.join("/dataset/nus_processed_CMRNet/sequences/scene-0922/map",
                             "map-scene-0922_0.1.pcd")
        ...
        self.Pcs = np.load("/dataset/nus_processed_CMRNet/sequences/scene-0922/Pc.npy",allow_pickle=True)
    ...
    def decode_meta(self, meta_info, seq):
        ...
        imp = os.path.join("/dataset/nus_processed_CMRNet/sequences/scene-0922/image",
                           "%06d" % int(seq) + ".jpg")
        # pcl = np.fromfile(pcp, dtype=np.float32).reshape(-1, 4)[:, :3]
        img = cv2.imread(imp)
        Pc = self.Pcs[int(seq)].numpy()
        K = np.asarray([[1266.417203046554, 0.0, 816.2670197447984],
    ...
    def vis(self):
        ...
        for vis_t in tqdm(range(self.num)):
            ...
            if video_project is None:
                video_project = cv2.VideoWriter(f"nus_project.mp4",
